...
```mermaid
flowchart LR
  P1["1. 事実と未知を確認"] --> P2["2. 20パラメータを設定"]
  P2 --> P3["3. 4ラウンドを自動実行"]
  P3 --> P4["4. 六軸と因果traceを確認"]
  P4 --> P5["5. 条件を変えて再実行"]
...
### build済み適応型Webでの人間とシミュレーターの役割

`frontend/dist`をbuild済みの適応型Webで人間が行うのは、20パラメータを設定して実行し、結果と因果traceを確認することです。
各年の行動を人間が手作業で決めるのではありません。ローカル決定論providerが5主体の行動候補を生成し、
決定論的coreが予算・人員・時間制約に基づいて採否し、2026年から2040年までの4ラウンドを実行します。

現在は次の三つの実行経路を分離しています。

- **適応型Web実行**: `frontend/dist`をbuild済みの場合、人間が設定した20パラメータから、5主体・4ラウンドの一つの適応型simulationを実行する
- **fallback Web実行**: fresh checkoutなど`frontend/dist`がない場合、tracked `web/` UIで固定fixture三分岐を実行・表示する。parameterや明示seedは受け付けない
- **三分岐run bundle**: 固定fixtureの国際統合型・国内自立型・開放基盤型を同一seedで実行し、replay可能な証拠を出力する

Webで編集した一組の条件から三分岐を同時に反実仮想実行する統合UIは未実装です。
外部AIもまだ接続せず、現在の提案生成と状態遷移はローカル決定論runtimeが所有します。
